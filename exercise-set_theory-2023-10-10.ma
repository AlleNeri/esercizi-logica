...
theorem in_to_subset: ∀A. (∃X. X ∈ A ) → ∃C. ( C ⊆ A ). (* tale C sarà il singoletto {X} *)
  ...
  by singleton we proved (∃singX. ∀Z. ((Z∈singX → Z=X) ∧ (Z=X → Z∈singX))) (exists_singleton_X)
  by singleton let SingX:set such that (∀Z. ((Z ∈ SingX → Z=X) ∧ (Z=X → Z ∈ SingX))) (singleton_X)
  we need to prove (SingX ⊆ A) (SingXinA)
    we need to prove (∀Z. Z ∈ SingX → Z ∈ A) (for_SingX_subset_A)
      assume Z: set
      suppose (Z∈SingX) (Z_in_SingX)
      by singleton we have (Z ∈ SingX → Z=X) (left_to_right) and (Z=X → Z ∈ SingX) (right_to_left)
      by singleton, Z_in_SingX we proved (Z = X) (ZeqX)
      by ZeqX, ax_extensionality done
 ...
 we need to prove (∀Z. Z ∈ A ∪ (B ∩ C) ⇔ Z ∈ (A ∪ B) ∩ (A ∪ C)) (II)
  ...
  we need to prove (Z ∈ (A ∪ B) ∩ (A ∪ C) → Z ∈ A ∪ (B ∩ C)) (I1)
   ...
   case or_intror
    ...
    we proceed by cases on Zor2 to prove (Z ∈ A ∪ (B ∩ C))
      case or_introl
        suppose (Z∈A) (H)
        by H, ax_union2, or_introl done
      case or_intror
        suppose (Z∈C) (H1)
        by H, H1, conj, ax_intersect2 we proved (Z∈B∩C) (K1)
        by K1, or_intror we proved (Z∈A∨Z∈B∩C) (K2)
  ...
  we need to prove (Z ∈ A ∪ (B ∩ C) → Z ∈ (A ∪ B) ∩ (A ∪ C)) (I2)
   ...
   case or_introl
    ...
    by K1, K2, conj, ax_intersect2 done
   case or_intror
    suppose (Z∈B∩C)(H)
    by H, ax_intersect1 we have (Z ∈ B) (H1) and (Z ∈ C) (H2)
    by H1, ax_union2, or_intror we proved (Z ∈ A∪B) (K1)
    by H2, or_intror, ax_union2 we proved (Z ∈ A ∪ C) (K2)
    by K1, K2, conj, ax_intersect2 done
  by I1, I2, conj done
 by ax_extensionality done
qed.
